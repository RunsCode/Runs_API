...
-(void) setBackgroundCircleLine:(ZFProgressViewStyle)style {
    ...
    if (style == ZFProgressViewStyleNone || style == ZFProgressViewStyleImageSegment) {
        ...
        for (int i = 0; i < ceil(360 / _GapWidth *_Percentage)+1; i++) {
            ...
            UIBezierPath *path1 = [UIBezierPath bezierPathWithArcCenter:CGPointMake(self.center.x - self.frame.origin.x, self.center.y - self.frame.origin.y)
                                                                 radius:(self.bounds.size.width - _backgourndLineWidth)/ 2 - _offset
                                                             startAngle:-M_PI_2 +(i *_GapWidth * M_PI / 180.0) endAngle:-M_PI_2 + angle clockwise:YES];
...
-(void)setProgressCircleLine:(ZFProgressViewStyle)style {
    ...
    if (style == ZFProgressViewStyleNone || style == ZFProgressViewStyleImageSegment) {
        path = [UIBezierPath bezierPathWithArcCenter:CGPointMake(self.center.x - self.frame.origin.x, self.center.y - self.frame.origin.y)
                                          ...
                                          startAngle:-M_PI_2 endAngle:-M_PI_2 + M_PI *2 clockwise:YES];
    } else {
        static float minAngle = 0.0081;
        for (int i = 0; i < ceil(360 / _GapWidth *_Percentage)+1; i++) {
            CGFloat angle = (i * (_GapWidth + minAngle) * M_PI / 180.0);
...
//    NSArray * colorTwo = [NSArray arrayWithObjects:/*(id)UIColorFromRGB(0xfde802).CGColor,*/(id)UIColorFromRGB(0xff757c).CGColor,(id)_progressStrokeColor.CGColor, nil];//0xfde802
    NSArray * colorTwo = [NSArray arrayWithObjects:(id)_progressStrokeColor.CGColor,(id)_progressStrokeColor.CGColor, nil];//0xfde802
    ...
    [gradientLayer2 setLocations:@[@0.5,@0.9,@1]];//@[@0.1,@0.5,@1]
