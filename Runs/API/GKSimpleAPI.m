...
+ (UIImage *)thumbnailImageForVideo:(NSURL *)videoURL atTime:(NSTimeInterval)time {
    ...
    if (!thumbnailImageRef)
        RunsLog(@"thumbnailImageGenerationError %@", thumbnailImageGenerationError);
...
+ (void)compressVideoWithUrl:(NSURL *)videoUrl completed:(void (^) (NSData * data))callback {
    RunsLog(@"开始压缩,压缩前大小 %f MB",[NSData dataWithContentsOfURL:videoUrl].length/1024.00/1024.00);
...
//                NSArray * array  = asset.metadata;
                RunsLog(@"压缩完毕,压缩后大小: %f MB",data.length/1024.00/1024.00);
                if (callback) {
                    callback(data);
                }
            }else{
#ifdef DEBUG
                RunsLog(@"当前压缩进度:%f",exportSession.progress);
                NSError *exportError = exportSession.error;
                RunsLog(@"AVAssetExportSessionStatusFailed: %@", exportError);
#endif
                if (callback) {
                    callback(nil);
                }
